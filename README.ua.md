...
Ви готові до нового етапу! Це друге домашнє завдання принесе вам більше практичних навичок у роботі з Node.js.

Під час виконання цього домашнього завдання ви створите сервер для роботи з колекцією контактів через HTTP запити. Ви почнете з ініціалізації проєкту, налаштуєте сервер, під'єднаєтеся до MongoDB та створите роути для роботи з контактами. Не забудь задеплоїти свій додаток на [render.com.](https://render.com/)
...
- Налаштування [cors](https://www.npmjs.com/package/cors) та логгера [pino.](https://github.com/pinojs/pino-http)
...
При вдалому підключенні до вашої бази в MongoDB виводьте в консоль рядок:
...
У файлі src/index.js викличте функцію initMongoConnection. Переконайтеся, що зʼєднання із базою встановлюється до запуску серверу.
...
Імпортуйте базовий набір контактів із файлу contacts.json до вашої бази, користуючись будь-яким UI-інтерфейсом (у браузері, MongoDB Compass тощо). Переконайтеся, що назва колекції в коді моделі та у візуальному інтерфейсі співпадають.
...
- Створення сервісу в папці src/services у файлі із відповідним іменем сутності (у даному випадку contacts.js)
...
- Реєстрацію роута в файлі src/server.js
- Опис контролера для цього роута
- Створення сервісу в папці src/services у файлі з відповідним іменем сутності (у даному випадку contacts.js)
...
Дуже важливо перед здачею домашнього завдання на перевірку ментору перевірити роботу вашого задеплоєного додатка на render.com.

Якщо, наприклад, під час деплою ви забули додати змінні оточення (env), то задеплоєний бекенд не буде працювати. Також перевірте, що всі створені вами маршрути бекенду працюють так, як очікується згідно із завданням.
